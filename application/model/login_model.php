	...
	class LoginModel {
		...
		public function registerNewUser(){
			...
			elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_name'])){
				$_SESSION['feedback_negative'] [] = FEEDBACK_USERNAME_NOT_ALLOWED;
			}
			...
			elseif (!filter_var($_POST['user_email'], FILTER_VALIDATE_EMAIL)) {
				$_SESSION['feedback_negative'] [] = FEEDBACK_EMAIL_NOT_VALID;
			}
			elseif (!empty($_POST['user_name']
				...
				$sql = "INSERT INTO users (user_name, user_password_hash, user_email, user_creation_timestamp, user_creation_timestamp                
				...
				if ($this->sendVerificationEmail($user_id, $user_email, $user_activation_hash)) {
					...
					$query = $this->db->prepare("DELETE FROM users
												 WHERE user_id = :last_inserted_id");
